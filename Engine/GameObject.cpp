...
float GameObject::GetAngleFromVector(const XMFLOAT2& vec, const bool& isConvertDeg)
{
	float angleRad = atan2(vec.x, vec.y);

	if (isConvertDeg)//角度に変換(falseなら変換せずラジアンのまま返す)
	{
		angleRad *= RAD_TO_DEG;
		if (angleRad < 0 && isConvertDeg) {
			angleRad += 360.0f;
		}
	}
	return angleRad;
...
float GameObject::CalculationDist(const XMFLOAT3& My, const XMFLOAT3& Target)
{
	return sqrt(((Target.x - My.x) * (Target.x - My.x)) +
		        ((Target.y - My.y) * (Target.y - My.y)));
}
