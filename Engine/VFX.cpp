...
int VFX::Start(EmitterData emitterData)
{
    ...
    pEmitter->pBillBoard->Load(emitterData.imageFileName);
...
EmitterData VFX::SetEffect(std::string FileName)
{
    CsvReader csv;//データを読むクラスのインスタンスを作成
    bool ret = csv.Load("CsvData\\Effect\\" + FileName);
    assert(ret);

    int row = 1,column = 0;
    EmitterData data;
    data.imageFileName = csv.GetString(row, csv.ReturnAndAddIndex(column));
    data.position      = csv.extractFloat3(csv.GetString(row, csv.ReturnAndAddIndex(column)));
    data.positionRnd   = csv.extractFloat3(csv.GetString(row, csv.ReturnAndAddIndex(column)));
    data.direction     = csv.extractFloat3(csv.GetString(row, csv.ReturnAndAddIndex(column)));
    data.directionRnd  = csv.extractFloat3(csv.GetString(row, csv.ReturnAndAddIndex(column)));
    data.speed         = csv.extractFloat(csv.GetString(row, csv.ReturnAndAddIndex(column)));
    data.speedRnd      = csv.extractFloat(csv.GetString(row, csv.ReturnAndAddIndex(column)));
    data.accel         = csv.extractFloat(csv.GetString(row, csv.ReturnAndAddIndex(column)));
    data.gravity       = csv.extractFloat(csv.GetString(row, csv.ReturnAndAddIndex(column)));
    data.color         = csv.extractFloat4(csv.GetString(row, csv.ReturnAndAddIndex(column)));
    data.deltaColor    = csv.extractFloat4(csv.GetString(row, csv.ReturnAndAddIndex(column)));
    data.rotate        = csv.extractFloat3(csv.GetString(row, csv.ReturnAndAddIndex(column)));
    data.rotateRnd     = csv.extractFloat3(csv.GetString(row, csv.ReturnAndAddIndex(column)));
    data.spin          = csv.extractFloat3(csv.GetString(row, csv.ReturnAndAddIndex(column)));
    data.size          = csv.extractFloat2(csv.GetString(row, csv.ReturnAndAddIndex(column)));
    data.sizeRnd       = csv.extractFloat2(csv.GetString(row, csv.ReturnAndAddIndex(column)));
    data.scale         = csv.extractFloat2(csv.GetString(row, csv.ReturnAndAddIndex(column)));
    data.lifeTime      = csv.extractFloat(csv.GetString(row, csv.ReturnAndAddIndex(column)));
    data.delay         = csv.extractFloat(csv.GetString(row, csv.ReturnAndAddIndex(column)));
    data.number        = csv.extractFloat(csv.GetString(row, csv.ReturnAndAddIndex(column)));
    data.isBillBoard   = csv.GetBool(csv.GetString(row, csv.ReturnAndAddIndex(column)));
