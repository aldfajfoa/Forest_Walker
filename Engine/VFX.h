...
struct EmitterData
{
	std::string imageFileName; //画像ファイル名
	...
	bool	 isBillBoard;	//ビルボードかどうか(trueの場合、常にカメラの方を向くパーティクルになる)

	//初期化
	EmitterData()
	{
		imageFileName = "Effect\\defaultParticle.png";
...
namespace VFX
{
	...
	EmitterData SetEffect(std::string FileName);
};
