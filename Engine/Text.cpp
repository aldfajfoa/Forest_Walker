...
//描画（文字列）
void Text::Draw(int x, int y, const char* str, float size)
{
	//表示位置（左上）を計算
	...
	for (int i = 0; str[i] != '\0'; i++)	//文字列の末尾まで来たら終わり
	{
		...
		//次の位置にずらす
		px += (width_ / (float)(Direct3D::screenWidth_ / 2.0f))* size;
	}
}

//描画（整数値）
void Text::Draw(int x, int y, int value, float size)
{
	//文字列に変換
	char str[256];
