...
protected:
	//定数
	const float PI = 3.14159265f;//円周率
	const float DEG_TO_RAD = PI / 180.0f;//角度をラジアンに
	const float RAD_TO_DEG = 180.0f / PI;//ラジアンを角度に
	...
	bool isSet_ = true;
	bool isEnd_ = false;//ゲームオーバーやゴールでゲームが終了したらtrueにしてオブジェクトの動きを止める

	//その他関数
	void Blinking(bool& isDraw) { isDraw = !isDraw; }
	//ベクトルを元に角度を返す
    //isConvertDegは角度に変換するかどうか
	float GetAngleFromVector(const XMFLOAT2& vec, const bool& isConvertDeg);
...
public:
	...
	//二点間の距離を計算
	float CalculationDist(const XMFLOAT3& My, const XMFLOAT3& Target);
	...
	void SetPosition(float x, float y) { SetPosition(XMFLOAT3(x, y, transform_.position_.z)); }
